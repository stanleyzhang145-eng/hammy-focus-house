...
async function handleApi(req,res,url){
  if(req.method==="GET"&&url.pathname==="/api/health/ready"){
    ...
    return json(res,200,{ok:true,version:24.9,databaseReady:true,databaseMode:db.mode()});
  }

  if(req.method==="GET"&&url.pathname==="/api/health"){
    return json(res,200,{
      ok:true,name:"Hammy Cloud server",version:24.9,
...
(async()=>{
  ...
  server.listen(PORT,HOST,()=>console.log(`Hammy Focus House v24.9 running at http://${HOST}:${PORT}`));
})();
