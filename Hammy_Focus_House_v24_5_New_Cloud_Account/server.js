...
async function handleApi(req,res,url){
  if(req.method==="GET"&&url.pathname==="/api/health/ready"){
    ...
    return json(res,200,{ok:true,version:24.5,databaseReady:true,databaseMode:db.mode()});
  }

  if(req.method==="GET"&&url.pathname==="/api/health"){
    return json(res,200,{
      ok:true,name:"Hammy Cloud server",version:24.5,
...
(async()=>{
  ...
  server.listen(PORT,HOST,()=>console.log(`Hammy Focus House v24.5 running at http://${HOST}:${PORT}`));
})();
