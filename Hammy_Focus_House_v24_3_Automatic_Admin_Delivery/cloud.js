...
(() => {
  ...
  async function api(path,options={},attempt=0){
    ...
    if(!response.ok){
      if(response.status===429&&attempt<1&&options.retry!==false){
        const retrySeconds=Math.max(
          1,
          Math.min(5,Number(data.retryAfterSeconds||response.headers.get("Retry-After"))||1)
        );
  ...
  async function backgroundRefresh(){
    ...
    try{
      ...
      if(error.status!==429)setChip("offline","Retrying");
    }finally{
