...
function apiRateLimit(req,url){
  // Static pages and files are never placed behind the application limiter.
  if(!url.pathname.startsWith("/api/"))return null;

  // These endpoints must remain reachable for deployment checks and login.
  // Admin login has its own incorrect-password limiter.
  if(
  ...
  const limit=isWrite?120:600;
  const key=`${clientAddress(req)}:${category}`;
  let entry=rateMap.get(key);

  if(!entry||current-entry.start>=windowMs){
    entry={start:current,count:1};
  ...
  return {
    retryAfterSeconds:Math.max(1,Math.ceil((entry.start+windowMs-current)/1000)),
    limit,
    category
  };
}

function cleanRateLimitEntries(){
  const cutoff=Date.now()-(5*60_000);
  for(const [key,entry] of rateMap){
    if(Number(entry.start||0)<cutoff)rateMap.delete(key);
  }
}
...
async function handleApi(req,res,url){
  if(req.method==="GET"&&url.pathname==="/api/health/ready"){
    ...
    return json(res,200,{ok:true,version:24.3,databaseReady:true,databaseMode:db.mode()});
  }

  if(req.method==="GET"&&url.pathname==="/api/health"){
    return json(res,200,{
      ok:true,name:"Hammy Cloud server",version:24.3,
...
const server=http.createServer(async(req,res)=>{
  ...
  const rateLimit=apiRateLimit(req,url);
  if(rateLimit){
    return json(
      res,
      429,
      {
        error:`Too many API ${rateLimit.category} requests. Try again in ${rateLimit.retryAfterSeconds} seconds.`,
        retryAfterSeconds:rateLimit.retryAfterSeconds,
        requestLimit:rateLimit.limit
      },
...
const rateLimitCleanupTimer=setInterval(cleanRateLimitEntries,5*60_000);
...
(async()=>{
  ...
  server.listen(PORT,HOST,()=>console.log(`Hammy Focus House v24.3 running at http://${HOST}:${PORT}`));
})();
